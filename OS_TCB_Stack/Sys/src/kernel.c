...
#define STACK_SIZE     128 // bao nhiêu ô stack
...
// ====== Stack Init Function ======
/*
* ban đầu cho SP trỏ vào R11
* pop R4 - R11
* Stack frame tự động khôi phục
*/
...
void osInit(void){
    CurrentPt = &tcbs[0]; // báo cho kernel biết sẽ khởi động từ task 0
    log_stack(CurrentPt->stackPt);
    Sys_Init();
    osSchedulerLaunch(); // file hợp ngữ chạy task đầu tiên, xác định vùng nhớ stack qua stack pointer của struct của task
}